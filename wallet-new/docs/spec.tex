...
blocks. From the point of the abstract wallet specification we are primarily
...
In this section we discuss how the abstract metadata model can be instantiated
to encode the information required in order to be able to report transaction
history as provided by the current `V1' wallet REST API.
...
\subsection{Transaction resubmission and cancellation}
...
eventually give up. This state is external to, and can be entirely independent
from, the rest of the wallet state. The only interaction with the wallet
proper is that at some point the network layer may decide to give up on
a transaction, removing it from the $\mathit{pending}$ set even though it has
not been included in the blockchain yet. From the point of view of the model
...
We remove it from all checkpoints since it would be strange if a transaction
cancelled would be reintroduced after a rollback. Conversely, the user may
wish to prompt the wallet to try again even after the wallet gave up. From
the point of view of the wallet, however, this is simply a call to
$\mathsf{newPending}$.

By the logic of Section~\ref{sec:transaction_history}
(Figure~\ref{fig:transaction_state}), a cancelled transaction will be reported
as $\texttt{WontApply}$. Effectively, $\mathsf{cancel}$ becomes a secondary way
in which a transaction may go from $\texttt{Applying}$ to $\texttt{WontApply}$
(arrow $\mathsf{applyBlock}^*$), and $\mathsf{newPending}$ a secondary
way to get back from $\texttt{WontApply}$ to $\texttt{Applying}$ (arrow
$\mathsf{rollback}^*$).
...
\todo[inline]{Should we be more precise about resubmission at this level of
detail? Things like exponential back-off, cut-off point (1hr?) before
cancelling, etc.}
